...
var _ repositories.VenueRepository = (*VenueRepository)(nil)

type VenueRepository struct {
	connection  *pgxpool.Pool
	transaction *pgxtx.CtxGetter
}

func NewLocationRepository(db *pgxpool.Pool) *VenueRepository {
...
func (r *VenueRepository) Save(ctx context.Context, venue models.Venue) (models.Venue, error) {
	transaction := r.transaction.DefaultTrOrDB(ctx, r.connection)

	if venue.ID != 0 {
		// Find venue by id
		oldVenue, findVenueError := r.FindById(ctx, venue.ID)
		if findVenueError != nil {
			return models.Venue{}, findVenueError
		}
		...
		_, updateVenueError := transaction.Exec(ctx, updateVenueQuery, args...)
	...
	result := transaction.QueryRow(ctx, insertVenue, venue.Name, venue.Type, venue.Description, venue.ArtworkID, venue.Address, metroStations)
	...
	if err := result.Scan(&venue.ID); err != nil {
		...
		if errors.As(err, &pgerr) {
			switch pgerr.Code {
			case "23505":
				return models.Venue{}, repositories.ErrorLocationAlreadyExists
			}
		}
...
func (r *VenueRepository) findBy(ctx context.Context, option findOption) (models.Venue, error) {
...
WHERE l.%r = $1`
	...
	if err := row.Scan(
		...
		if errors.Is(err, pgx.ErrNoRows) {
			return models.Venue{}, repositories2.ErrorLocationNotFound
		}
		return models.Venue{}, err
	}
...
func (r *VenueRepository) FindById(ctx context.Context, id int) (models.Venue, error) {
...
func (r *VenueRepository) FindAll(ctx context.Context) ([]models.Venue, error) {
	type record struct {
		ID                   int
		Name                 string
